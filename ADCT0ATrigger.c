...
//  while((SYSCTL_PRADC_R&0x0001) != 0x0001){};    // good code, but not yet implemented in simulator
  ...
  ADC0_SSPRI_R = 0x0123;          // 8) Sequencer 3 is highest priority
  ADC0_ACTSS_R &= ~0x0008;        // 9) disable sample sequencer 3
  ADC0_EMUX_R &= 0x4000;         // 10) seq3 is software trigger
  ADC0_SSMUX3_R &= ~0x000F;       // 11) clear SS3 field
  ADC0_SSMUX3_R += 9;             //    set channel
  ADC0_SSCTL3_R = 0x0006;         // 12) no TS0 D0, yes IE0 END0
  ADC0_IM_R &= ~0x0008;           // 13) disable SS3 interrupts
  ADC0_ACTSS_R |= 0x0008;         // 14) enable sample sequencer 3
	//init portd for GPIO interrupt	
	SYSCTL_RCGCGPIO_R |= 0x00000008; // (a) activate clock for port D
	while ((SYSCTL_RCGCGPIO_R & 0x08) != 0x08){ 		// do nothing
	}
  GPIO_PORTD_DIR_R &= ~0x40;    // (c) make PF4 in (built-in button)
  GPIO_PORTD_AFSEL_R &= ~0x40;  //     disable alt funct on PD4
  GPIO_PORTD_DEN_R |= 0x40;     //     enable digital I/O on PD4   
  GPIO_PORTD_AMSEL_R &= ~0x40;       //     disable analog functionality on PF
  GPIO_PORTD_PDR_R |= 0x40;     //     enable weak pull-up on PF4
  GPIO_PORTD_IS_R &= ~0x40;     // (d) PF4 is edge-sensitive
  GPIO_PORTD_IBE_R &= ~0x40;    //     PF4 is not both edges
  GPIO_PORTD_IEV_R &= ~0x40;    //     PF4 falling edge event
  GPIO_PORTD_ICR_R = 0x040;      // (e) clear flag4
  GPIO_PORTD_IM_R |= 0x40;      // (f) arm interrupt on PF4 *** No IME bit as mentioned in Book ***
  NVIC_PRI0_R = (NVIC_PRI7_R&0x1FFFFFFF)|0x40000000; // (g) priority 
  NVIC_EN0_R |= 0x08;      // (h) enable interrupt 30 in NVIC
	GPIO_PORTD_ADCCTL_R |= 0x4;
